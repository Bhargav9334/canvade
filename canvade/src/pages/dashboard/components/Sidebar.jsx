...
const menuItems = [
  ...
  { id: 'enrollments', label: 'My Enrollments',        icon: MessageSquare,   sub: 'Create and Manage your Courses and Workshops' },
  { id: 'payments',    label: 'Revenue & Payments',    icon: Coins,           sub: 'Revenue, Payment, Withdrawal and History' },
  { id: 'profile',     label: 'Profile & Verification',icon: User,            sub: 'Update your institute profile' },
  ...
  return (
    <aside className="w-[280px] h-full shrink-0 bg-white border border-slate-100 rounded-2xl p-4 flex flex-col gap-2 shadow-sm overflow-y-auto no-scrollbar">
      <div className="flex items-center justify-between lg:hidden mb-2">
        <span className="text-sm font-bold text-slate-700 font-[Poppins]">Menu</span>
        <button
            ...
            <div key={item.id} className="w-full flex flex-col gap-1">
              ...
              <button
                ...
                className={`w-full text-left px-4 py-3 rounded-xl transition-all duration-200 group flex items-center gap-3 ${
                  isActive
                    ? 'bg-[#1fa485] text-white shadow-sm'
                    : 'bg-[#f1f1f1] hover:bg-slate-200/80 text-slate-700'
                }`}
              >
                <item.icon
