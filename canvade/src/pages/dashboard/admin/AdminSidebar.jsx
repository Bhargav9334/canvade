...
const adminMenuItems = [
  ...
  },
  {
    ...
    icon: BookOpen,
    sub: "Create and Manage your Courses and Workshops",
  },
  {
    id: "payments",
        ...
        {adminMenuItems.map((item) => {
          ...
          if (isActive) {
            if (item.id === "promotions") {
              currentBtnStyle = "bg-[#F3C720] text-white shadow-sm";
            } else {
              currentBtnStyle = "bg-[#1D9B7E] text-white shadow-sm";
            }
          } else {
            if (item.id === "promotions") {
              currentBtnStyle =
                "bg-[#f1f1f1] text-slate-700 hover:bg-[#F3C720] hover:text-white";
            } else {
              currentBtnStyle =
                "bg-[#f1f1f1] text-slate-700 hover:bg-[#F3C720] hover:text-white";
            }
          ...
          return (
            <div key={item.id} className="w-full flex flex-col gap-1">
              <button
                ...
                className={`w-full text-left px-4 py-2.5 rounded-xl transition-all duration-200 group flex items-center gap-3 ${currentBtnStyle}`}
              >
                <item.icon
